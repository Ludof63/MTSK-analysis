...
time_differences AS (
    SELECT
        station_uuid,
        EXTRACT(EPOCH FROM (time - previous_time)) AS sec_between_updates
...
station_frq AS (
    SELECT
        station_uuid,
        AVG(sec_between_updates)/60 AS avg_min_between_updates
...
frq_stats AS (
    STDDEV(avg_min_between_updates) as stddev_frq,
    AVG(avg_min_between_updates) as mean_frq,
)

stations_with_fr AS (
    SELECT f.station_uuid, avg_min_between_updates, city, brand, latitude, longitude
    FROM station_frq f, stations s
    WHERE f.station_uuid = s.id
)
select * from stations_with_fr;