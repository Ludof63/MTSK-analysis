...
DIESEL_QUERY="../sql/queries/diesel.sql"    #relative path
...
def station_count_by_price_range(city : str, date : datetime, fuel : str = 'diesel'):
    ...
    with pg.connect(f"host=localhost port={PORT} user={USER} password={USER_PSWD} dbname={DB}") as conn:
        df = pd.read_sql(query,conn, params=(date,city,))
...
def main():
    ...
    station_count_by_price_range(sys.argv[1].title(),DATE,sys.argv[2])
